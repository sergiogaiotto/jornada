...
export const CONTEUDO_GUIA: Record<ChaveGuia, ConteudoPagina> = {
  ...
  validacao: {
    ...
    pegadinhas: [
      ...
      "\"Abrir pendência…\" pede contexto ANTES do POST (A9): título obrigatório (vem prefillado, edite), descrição e severidade opcionais — Esc ou clique-fora cancelam sem criar nada.",
  ...
  criativo: {
    ...
    pegadinhas: [
      ...
      "O KV de partida é DERIVADO do briefing desta OS (A8, determinístico — via_ai:false), nunca copy de outra campanha; briefing magro vira placeholder \"(defina o Key Visual)\", que é convite para você escrever, não sugestão do agente.",
    ],
  ...
  twin: {
    ...
    campos: [
      ...
      { campo: "Paleta (modo Desenho)", significado: "Os 16 tipos de nó do JGC agrupados como no JB — arraste para criar; cada nó nasce com dados default válidos." },
      { campo: "Inspetor de nó", significado: "Dados do nó por tipo + prévia do JSON SFMC que o compilador gerará." },
      { campo: "Versões (header)", significado: "Linha do tempo do twin: v1 · publicado, v2 · rascunho… Versão antiga abre somente leitura, com \"Restaurar como nova versão\"." },
      { campo: "Comparar com… (diff visual)", significado: "Pinta o diff no próprio canvas: anel verde = adicionado, fantasma vermelho = removido, âmbar = alterado; painel lateral clicável centraliza a mudança." },
      { campo: "Exportar", significado: "JSON = spec de import NATIVO do Journey Builder; XML = mesma spec com manifest (hash/versão/geradoEm) para integração e auditoria." },
    ...
    casosDeUso: [
      ...
      "Voltar numa versão antiga da linha do tempo, comparar com a atual e restaurar como novo rascunho.",
      "Exportar a jornada (JSON para importar no JB; XML para auditoria) com hash no manifest.",
      "No modo Simulação, ver os volumes P50 da última simulação pintados por nó e aresta.",
    ],
    exemplo: [
    ...
    pegadinhas: [
      "Salvar INVALIDA a simulação: o estado volta a rascunho e o Ensaio anterior deixa de valer — re-simule antes de avançar (o snapshot amarra tudo por hash).",
